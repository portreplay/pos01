	...
	if(isset($_GET['ref']) and !empty($_GET['ref']) and $_GET['ref'] == 'add') {
		if(isset($_POST['kd_barang']) and !empty($_POST['kd_barang'])) {
			...
			foreach ($kode_barang as $kd_barang) {
				...
				$get_info_barang = $mysqli->query("SELECT * FROM tbl_stok_toko INNER JOIN tbl_barang ON tbl_barang.kd_barang=tbl_stok_toko.kd_barang WHERE tbl_stok_toko.kd_barang='$kd_barang'");
				$data_info_barang = $get_info_barang->fetch_array();

				$nama_barang = $data_info_barang['nm_barang'];
				$harga = $data_info_barang['harga']+$data_info_barang['biaya_expedisi'];
